...
var array_path_elements;
var array_path_recoding_started;

let transformer = {
    transform: function (fromPayload, toPayload, dataDictonary, functions) {
        dataDictonary.forEach(mapping => {
            ...
            array_path_elements = []
            array_path_recoding_started = true
            updateObject(mapping.source, mapping.target, fromPayload, toPayload, func)
        });
    }
}

let updateObject = function (sourcePath, targetField, fromPayload, toPayload, func) {
    var path = jp.paths(fromPayload, '$.' + sourcePath)
    if (path && path.length > 0) {
        var stop_further_proc = false;
        var processed_nodes = [];
        var processed_arrays = {};
        path[0].forEach((node, index) => {
            if (node != '$' && !stop_further_proc) {
                processed_nodes.push(node)
                var value = jp.query(fromPayload, util.createJpExp(processed_nodes, processed_arrays));
                var node_type = util.findType(value[0])
                if (node_type == 'String' || index == path[0].length - 1) {
                    setValue(toPayload, targetField, value[0], func)
                } else if (node_type == 'Array') {
                    array_path_recoding_started = false
                    stop_further_proc = true;
                    processed_arrays[node] = true;
                    if (util.findType(value[0][0]) == 'String') {
                        setValue(toPayload, targetField, value[0], func)
                    } else {
                        array_path_elements.forEach(elm => {
                            if (!toPayload[elm])
                                toPayload[elm] = {}
                        ...
                        value[0].forEach((val, indx) => {
                            ...
                            updateObject(util.shrinkJpExp(sourcePath, processed_nodes), targetField, val, elem)
                        })
                    }
                } else if (util.findType(value[0]) == 'Object') {
                    if (!array_path_recoding_started)
                        array_path_elements.push(node)
                }
            }
        })
    }
...
let setValue = function (obj, path, value, func) {
    ...
    for (var i = 0; i < parts.length; i++) {
        if (i < parts.length - 1) {
            ...
            obj[parts[i]] = func ? func(value) : (value ? value : "")
        }
    }
}
...
var erepayload = {}
emappings = [{"source": "a.b.c[*]", "target" : "x.y.z", "function": null}]
erpayload = {"a": {"b" : {"c": ["no", "yes", "go"]}}} 

transformer.transform(erpayload, erepayload, emappings, functions);

console.log(JSON.stringify(erepayload))
